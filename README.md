# Jitendex Russian translation pipeline

This repository implements the deterministic part of the workflow in
...
The active `lexicographer-v2` pipeline treats the English Jitendex article as
...
Runs are isolated in the same SQLite database. `run.pipeline_version`, prompt
...
Requires Python 3.12+, `zstd` for modern Anki packages, and the two pinned
...
Run `translationctl --help` for the complete staged workflow. Worker and review
...
The normal sequence after import is `resolve-scope`, review the unresolved JSON
...
`claim --batch-id ID` can reserve a specific ready batch for a focused pilot.

`acquire` also downloads the two Yomitan JSON schemas from an immutable upstream
