...
YRP-STATUS-8 — All local build, verification, documentation, and clean-profile Yomitan smoke gates now pass. The consolidated gate is `ready_for_draft`; publication starts at YRP-PUB-2.

YRP-STATUS-9 — `COMPLETE` as of 2026-08-21. Version `1.0.1`, all seven release assets, the owned update index, and the public download page are published and verified. YRP-PUB-2 through YRP-PUB-9 and the post-publication YRP-SMOKE-10 audit are done.
...
YRP-EXEC-45 — `DONE` for the permanent cross-format lexical gate. `scripts/audit_rich_export_archives.py` scanned the shared materialized source for all 433,885 articles, scanned every ZIP payload, and decompressed every MDict record. All five archives contain zero raw `redirected from ` and zero raw `valid only for these forms and/or readings`; the shared source has zero localization issues. Evidence is stored in `reports/exporters/run59-v1.0.1-rich-lexical-audit.json`. Commit `e630310` is `Prove every release archive uses the repaired Russian corpus` because format verification alone did not detect untranslated structural labels, and this gate prevents that regression across all exporters.

YRP-EXEC-46 — `STAGED` for the owned update index. `site-home/yomitan.json` was generated from final export 91, passed the pinned Yomitan index schema, and matched the archive on nine canonical metadata fields. The hosted file and archive `index.json` are byte-identical with SHA-256 `90017a061ee25d82c1f4fd312175d02c57a792ac1cdea295d21b3ee02e7638c7`. It uses revision `2026.08.21.0-jp-ru-kolobok-400k-v1.0.1-tags-ru-v1`, the owned Pages index URL, and the owned v1.0.1 release download URL. Evidence is `reports/yomitan_localization/run59-v1.0.1-update-index-verify.json`.

YRP-EXEC-47 — `STAGED` for release documentation and the public home page. README preserves the old v1.0 evidence and adds new v1.0.1 state IDs. Run history records the incident, approved repairs, backup, canonicalization count, builds, hashes, reproduction, and the permanent removal of full-corpus Apple `xmllint`. The home page points all five formats and structured data to v1.0.1 and tells v1.0 users to import it manually once.
...
YRP-EXEC-57 — `READY FOR USER CLICK-THROUGH` for YRP-SMOKE-1 through YRP-SMOKE-9. `site-home/yomitan-smoke.html` turns the exact 18-check contract into large Japanese Yomitan targets with archive-backed expectations, persistent local checkboxes, updater instructions, and an exact smoke-report JSON download. The release gate now requires this support page and verifies its check-key contract. The suite collected 172 tests: 170 passed and the same two documented PostgreSQL integration tests skipped. The consolidated gate still has five exact artifacts, zero findings, zero missing reports or support files, and status `manual_smoke_pending` until a person marks only observed UI results. Commit message: `Make Yomitan release smoke directly testable in the browser` because the remaining release barrier needs a low-friction human UI while preserving the fail-closed evidence contract.

YRP-EXEC-58 — `DONE` for YRP-SMOKE-1 through YRP-SMOKE-9. The user observed all 18 checks in a clean Chrome `151.0.7922.169` arm64 profile with Yomitan `26.7.29.0`. The update server received one index request and one final-ZIP request. The settings page first showed two identical final-revision rows, then showed one enabled `Колобок 400k` row after one reload. `reports/yomitan_localization/run59-v1.0.1-smoke-attempt-1.json` preserves the first observation; the successful report has SHA-256 `e648a70b48e415b085af33ba3dcf5b09a1eead7090ead7ca5f418862ad04d79d`. `record-yomitan-smoke` recorded export 91 and completed Run 59. The suite collected 173 tests: 171 passed and the same two documented PostgreSQL integration tests skipped. The consolidated gate has zero findings and status `ready_for_draft`. Commit message: `Record the observed Yomitan update without hiding its transient UI state` because publication needs positive human evidence and a truthful record of the temporary duplicate row.

YRP-EXEC-59 — `DONE` for YRP-PUB-2 through YRP-PUB-9. The draft release uploaded seven explicit assets and a clean reverse download matched all five archive hashes before publication. Public release `https://github.com/ganqqwerty/jp-ru-kolobok-dictionary/releases/tag/v1.0.1` uses tag `v1.0.1` at verified release commit `e8696066bd629ecd265783e5b5a3de43b933330b`. Concurrent homepage-card work from remote `main` was preserved in merge commit `02e1797b7b656194cbff2703cd4ac5c60397d3af`. Pages workflow `32482676304` completed successfully, and the temporary release branch was deleted after the tag and ancestry checks passed.

YRP-EXEC-60 — `DONE` for YRP-SMOKE-10 and YRP-PUB-DONE-1. The public `yomitan.json` is byte-identical to the staged file with SHA-256 `90017a061ee25d82c1f4fd312175d02c57a792ac1cdea295d21b3ee02e7638c7`; its title, revision, page URL, index URL, and download URL are Kolobok-owned and contain no operational Jitendex endpoint. All seven public assets were fetched into a new post-publication directory. The five archives matched the public checksum manifest, and the Yomitan, GoldenDict, MDict, PocketBook, and Apple verifiers all passed on the registered byte-identical files. Full-corpus `xmllint` was not run.

YRP-EXEC-61 — `DONE` for the final automated gate after the concurrent homepage merge. The suite collected 174 tests: 172 passed and two documented PostgreSQL integration tests skipped. Commit message: `Complete v1.0.1 publication with verifiable public provenance` because the plan must record public URLs, tag ancestry, Pages deployment, reverse-download hashes, and post-publication verifier results instead of stopping at a local release candidate.
